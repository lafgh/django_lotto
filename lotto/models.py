...
class GuessNumbers(models.Model):
    ...
    def generate(self): # 로또 번호를 자동으로 생성
        ...
        for _ in range(0, self.num_lotto):   # 0~4
    ...
    def __str__(self): # Admin page에서 display되는 텍스트에 대한 변경
        return "pk {} : {} - {}".format(self.pk, self.name, self.text) # pk(primary key)는 자동생성됨
        # return "pk {} : {} - {}".format(self.id, self.name, self.text)
